...
int main(int argc, char** argv){
	...
	topology.push_back(400);// Hidden nodes
	topology.push_back(62);// Output nodes 
	Net nnet(topology);


	/* Load the symbol -> value map, value -> symbol map and the histogram template values. */
	...
	/* Generate training data from the SD19 images stored in the data folder */
	if (argv[1][0] == 'g') {
	...
	/* Run the system using localized pooled thresholding */
	else if(argv[1][0] == 'z') {
		if (argc < 7) {
		...
		/* If the image is large, compress it to save time */
		rescale_image(image);
	    ...
	    /* Run the thresholding to convert to binary image */
		...
		std::cout<<"Runtime of binary conversion: "<<((float)t3-(float)t2)/CLOCKS_PER_SEC<<std::endl;
	 	...
	 	/* Read the letters and store the result in results.txt */
	...
	/* Run the system using canny edge detection */
	else if(argv[1][0] == 'c') {
		if (argc < 6) {
	    ...
	    /* Run the edge detection and store the results in edges */
